...
namespace LigthStringEditor {
    public class DatTL {
        ...
        public byte[] Export(string[] Strings) {
            ...
            string[] exportMalieLabels = new string[MalieLabels.Length];
            
            if (FilterEnabled) {
                // 필터 활성화: 원본 유지 + 수정본 덮어쓰기
                for (int i = 0; i < MalieLabels.Length; i++) {
                    exportMalieLabels[i] = MalieLabels[i];  // 원본 유지
                }
                
                for (int i = 0; i < filteredMalieLabelCount; i++) {
                    int originalIndex = filteredToOriginalMap[i];
                    exportMalieLabels[originalIndex] = Strings[i];  // 수정본 덮어쓰기
                }
            } else {
                // 필터 비활성화: 그대로 사용
                exportMalieLabels = Strings.Take(filteredMalieLabelCount).ToArray();
            }
